...
import com.example.flowsappp2.domain.WeatherState
    ...
    fun onSearchQueryChanged(query: String){
        ...
        if(query.lowercase() == "error"){
            viewModelScope.launch {
                _errorEvents.send(
                    "Manual Error Triggered: 'error' is a forbidden city!"
                )
            }
            return
        }
        ...
        searchJob = viewModelScope.launch {
            ...
            // Debounce used to reduce requests to the server, as they are made once user finishes typing
            //Using debounce to actually confirm if the user finished tipying

            repository.searchCities(query).onSuccess { cities ->
